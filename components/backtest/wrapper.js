...
class Wrapper {
  constructor({date,capital=300000,hightLoss=0.1,handlingFeeRebate=0.65,limitHandlingFee=20}){
    this.date = date;
    this.context = new Context({ subject: date, capital,hightLoss,handlingFeeRebate,limitHandlingFee });
  ...
  show(detail){
    ...
    let obj = {
      "進出點":"符合昨日技術分析，以今日開盤價買進。 符合賣出條件，以今日收盤價賣出",
      "本金":this.context.capital,
      "損益":this.context.profit,
      "Win":this.context.win,
      "Lose":this.context.lose,
      "勝率": `${Math.round(this.context.win/(this.context.win+this.context.lose)*1000)/10} %`,
      "未實現損益":profit,
    }
    if(detail){
      obj["目前持股"] = data;
