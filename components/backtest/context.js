...
const { BuyMethod, SellMethod } = require("./logic");
class Context {
  constructor({
    capital=300000,
    subject,
  ...
  buy() {
    ...
    */
    ...
    Object.keys(list).forEach((key) => {
      ...
      // // 符合標準就買入
      if (this.capital > 0 && !handle.hasOwnProperty(key) && response["o"]< 200) {
        if (response) {
  ...
  sell() {
    ...
    */
    ...
    Object.keys(handle).forEach((key) => {
      ...
      if (
        handle[key]['buy']-handle[key]['buy']*this.hightLoss > handle[key]['count']*response["c"] // 虧損大於設定:賣出
      ) {
